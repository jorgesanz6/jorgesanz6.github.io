...
export const projects = [
  {
    name: "Detección de ineficiencias de mercado con ML",
    description: "Sistema propio de pronóstico y detección de ineficiencias en mercados de predicción: un pipeline de datos y machine learning que estima resultados individuales, calibra la incertidumbre de cada pronóstico y lo contrasta contra la probabilidad implícita del mercado para encontrar dónde se equivoca.",
    problem: "La probabilidad implícita que fija un mercado de predicción no siempre refleja bien toda la información disponible —forma reciente, lesiones, contexto del enfrentamiento—. Encontrar esas discrepancias a mano, cruzando estadísticas históricas y datos de decenas de eventos cada día, no escala ni es sistemático.",
    approach: [
      "Pipeline propio de ingesta multi-fuente (series temporales de resultados, estadísticas históricas, incidencias) orquestado por cron en GitHub Actions: recogida de datos, inferencia, verificación de resultados y aviso automático por bot de mensajería corren solos en horarios fijos, de principio a fin, sin que yo toque nada.",
      "Motor de pronóstico ensemble de dos capas: XGBoost, LightGBM y CatBoost como modelos base, con un meta-modelo (red neuronal) entrenado sobre sus predicciones out-of-fold para combinarlos.",
      "Calibración conformal basada en residuos históricos en lugar de asumir una distribución a ciegas, para que el intervalo de incertidumbre de cada pronóstico sea honesto.",
      "Clasificador independiente que traduce cada pronóstico en una probabilidad calibrada de superar el valor de mercado (AUC ~0.89 en validación) — la pieza que detecta la ineficiencia, no solo la proyección.",
      "Verificación automática contra resultados reales y reentrenamiento semanal basado en el error: el sistema corrige sus propios modelos con cada nueva jornada de datos.",
      "Auditoría propia del pipeline de entrenamiento que encontró y corrigió fugas de datos (variables derivadas del propio resultado, escalado mal particionado en validación cruzada) — el tipo de error silencioso que infla las métricas sin que el modelo sirva de nada en producción.",
    ],
    tech: ["Python", "XGBoost / LightGBM / CatBoost", "Supabase", "GitHub Actions", "Telegram Bot API"],
    status: "En uso",
    url: null,
  },
  {
  ...
  {
    name: "Registro horario y calendario laboral",
    description: "PWA personal para fichar entradas y salidas, calcular horas extra y gestionar vacaciones y ausencias, con el calendario laboral de Madrid integrado. Instalable tanto en el móvil como en Windows.",
    problem: "Llevar el control de entradas, salidas, pausas de comida y días libres a mano —o repartido entre varias apps— hacía muy difícil saber cuántas horas extra llevaba acumuladas o cuántos días de vacaciones me quedaban en un momento dado.",
    approach: [
      "Cálculo automático de la hora de salida ideal cada día, con reglas distintas según el tipo de jornada: los viernes y los periodos de jornada intensiva (7h) no descuentan la pausa de comida del cálculo; el resto de días sí.",
      "Contador de horas extra semanal que excluye automáticamente semanas con festivo o vacaciones en viernes, y marca en verde cuando se superan los 60 minutos de exceso.",
      "Gestión de vacaciones, permisos y bajas con contador anual de días consumidos y disponibles, excluyendo fines de semana y festivos del cómputo para no descontar días que no tocan.",
      "Carga automática por año de los festivos nacionales y del calendario laboral de Madrid, sin introducirlos a mano uno a uno.",
      "Migración de un histórico de fichajes en Excel: un script propio parsea el XML interno del archivo directamente, sin librerías de terceros, para manejar bien el bug de fecha de 1900 de Excel antes de subir los datos a Supabase.",
      "PWA instalable en móvil y en Windows, con service worker para funcionar sin conexión. Pensada para escalar: hoy la uso solo yo, pero la estructura ya contempla gestión de usuarios o calendarios compartidos para un equipo.",
    ],
    tech: ["JavaScript", "Supabase", "PWA / Service Worker", "Python"],
    status: "En uso",
    images: [
      "/projects/registro-horario/01-calendario.png",
      "/projects/registro-horario/02-fichajes.png",
    ],
    url: null,
  },
]
...
// Derivado de los arrays de arriba para que nunca se desincronice al añadir
// un proyecto o una certificación nueva.
export const stats = [
  { key: "rendimiento_dashboards", value: "+60%", label: "rendimiento en dashboards clave" },
  { key: "años_trayectoria", value: "+7", label: "años entre banca, análisis financiero y datos" },
  { key: "años_en_bi", value: "+3", label: "años en Business Intelligence" },
  { key: "proyectos_propios", value: `${projects.length}`, label: "proyectos propios construidos y en uso" },
  { key: "certificaciones_6meses", value: `${certifications.length}`, label: "certificaciones en 6 meses" },
]
